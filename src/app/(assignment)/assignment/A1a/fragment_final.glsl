...
vec3 starTrail(in vec3 p)
{
    ...
    float t = time_cycle * 10.0;
    vec3 color = vec3(0.0);
    
    for (int i = 0; i < 10; i++) {
        vec3 p = rotateXYZ(p, vec3(0.0), vec3(0.0, 0.0, t - float(i) * 0.6));
...
vec3 phongShading(
    ...
    switch (hit_id.id) {
        ...
        case 7: // River
            ...
            // float water_color_adjust = 1.0 - p.z / 50.0;
        ...
        case 8: // Tree
            color = vec3(1.0);
            if (p.y > 0.5) {
                color *= mix(1.0, 1.5, p.y - 0.5);
            }
            break;

        case 9: // Gate
            color = vec3(0.76, 0.55, 0.55);
            // if (int(p.y * 7.0) % 2 == 0 && p.y > 1.2) {
            //     color *= 0.5;
            // }
            break;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    float brightness_scale = 1.4 + 0.1 * time_cycle * 3.0;
    ...
    light.shininess = 128.0;

    vec4 color = rayMarchVolume(origin, dir, light, brightness_scale); // Call the cloud shader
